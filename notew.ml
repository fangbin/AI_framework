...
module Model = struct
  ...
  and do_assign_one (eid: MEV.t) (esh: MSH.t) (dw: MDW.t)
    ...
    begin
      ...
      else if ((List.length !nllv) == (List.length !nlexp))
      then 
      		begin 
      			(* check left is symbolic vars of not  *)
      			let nm = ref (ModelMap.empty) in 
      			...
      			then 
      			(
      				let ndw = MDW.do_assign dw !nllv !nlexp in 
      				nm :=  (ModelMap.singleton esh ndw);
      				on_data := true
      			)
      			else () 
                                      ...
                                      ( 
                                        fun (nsh, _vl, cl) -> 
                                          ...
                                          then  
                                            ...
                                            let _ = Mman_options.Self.debug ~level:2 "MV:shape is not changed...@."  in 
