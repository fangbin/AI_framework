...
let env2apron (eid: Mman_env.t)
  ...
  =
    (*let _ =
      ...
      (
        fun ei apei ->
          Mman_options.Self.debug ~level:2 "DW:old apron envs list: eid%d %a @."
          ei
    ...
    with Not_found ->
      ...
      let avl =
          ...
          (
            fun (_i,sv) -> 
              (*let _ =  Mman_options.Self.debug ~level:2 "%a,  @." 
...
let to_tcons (sei: Mman_env.t) (ac: Mman_asyn.aconstr)
  ...
  | Mman_asyn.ACmp(op, aeL, aeR) ->
      ...
      let _ = Mman_options.Self.debug ~level:1
          "  const:%a@."  Apron.Tcons1.print res
      in res
...
module Model = struct
  ...
  *)
  ...
  let intersects (d0: t) (d1: t) =
    let _ = Mman_options.Self.debug ~level:2 "DW:compute intersection..."
    in
    (*let m = Apron.Abstract1.meet man_apron
                      (to_apron d0.vap) (to_apron d1.vap)
    in*)
    let m = Apron.Abstract1.join man_apron
                      (to_apron d0.vap) (to_apron d1.vap)
    in 
    let v =
      { eid=d0.eid;
    ...
    in
    let _ = Mman_options.Self.debug ~level:2 "DW:after intersection:\n %a"
  ...
  let meet_exp (sei: Mman_env.t) (d: t) (c1_cn: Mman_asyn.aconstr list) =
    ...
    let _ = Mman_options.Self.debug ~level:1 "DW:meet_exp....(pei:%d)@." sei in
    let apenv = Apron.Abstract1.env (to_apron d.vap) in
  ...
  let do_assign (d: t) (llv: Mman_asyn.alval list) (lexp: Mman_asyn.aexp list)
    ...
    let _ = Mman_options.Self.debug ~level:1 "DW:do_assign: peid:%d@." d.eid
    in
  ...
  let change_env (d: t) (eiold: Mman_env.t) (einew: Mman_env.t)
    ...
    (*let _ = Mman_options.Self.debug ~level:2 "DW:eid:%d, eiold:%d, einew: %d@." d.eid eiold einew in *)
    if eiold == einew
    then
      let _ = Mman_options.Self.debug ~level:2 "DW:emvironment changed @." in
      copy_intern d
    else
      (*assert (d.eid == eiold)) *)
